...
class ComparisonManager:
    ...
    def _progress(self, desc: str, total=None, transient: bool = True):
        """Yield (progress, task_id) using logger's make_progress, or (None, None)."""
    ...
    def _job_hash(self, dataset_path: str, target: str,
                  epoch_count: int, max_x_list: list) -> str:
        key = json.dumps({
    ...
    def _init_checkpoint(self, dataset_path: str, target: str,
                         epoch_count: int, max_x_list: list):
    ...
    def run_all(
        ...
        # ── Resolve max_x list early so checkpoint hash is stable ─────
        max_x_values = (
            segment_max_x
        ...
        if isinstance(dataset, str):
            ...
            with self._progress(f"[cyan]Loading CSV…", transient=True) as (prog, task):
        ...
        with self._progress(f"[cyan]Preprocessing {n_rows} rows…", transient=True) as (prog, task):
        ...
        # De-normalise the target column back to original scale.
        # PreProcessingNode normalises all numeric columns including the target;
        # sklearn models have no knowledge of this and compute MAPE against the
        # raw-scale target, so we must undo it here.  Feature columns stay
        # normalised — all models handle that uniformly.
        # SegmentHandler re-normalises the target internally via its own
        # preprocessor run, so it is unaffected.
        tb = preprocessor.feature_bounds.get(target)
        if tb is not None:
            lo, hi = tb
            span = hi - lo
            if span > 1e-9:
                processed_df[target] = processed_df[target] * span + lo
                self._log(
                    f"Target '{target}' de-normalised to original scale "
                    f"[{lo:.3g}, {hi:.3g}] for baseline models."
                )
    ...
    def _save_comparison_csv(self, run_id: str):
        """Append all results from this run to comparison_results.csv."""
        n = len(self.results)
        self._log(f"Writing {n} rows to {COMPARISON_CSV}…")
        with self._progress(
            f"[cyan]Saving {n} rows → {COMPARISON_CSV}",
        ...
        self._log(f"Saved → {COMPARISON_CSV}")
    ...
    def _rich_report(self, console):
        ...
        console.print(t)
        console.print(
            f"\n[dim]Full results saved to [bold]{COMPARISON_CSV}[/bold][/dim]\n"
        )
