...
class Main:
    ...
    def initialize_nexus(self, Loud):
        ...
        with tqdm(total=3, desc="Nexus Initialization", leave=True) as nexus_pbar:
            ...
            for i, segment in enumerate(self.segments):
                bar = tqdm(total=len(segment['processor']), desc=f"Segment {segment['id']} Connecting", position=i)
                bars.append(bar)
            with concurrent.futures.ThreadPoolExecutor() as executor:
                for i, segment in enumerate(self.segments):
                    bar = bars[i]
    ...
    def infer(self, input, loud):
        ...
        with tqdm(total=4, desc="Inference", leave=True, disable=disable_bars) as inference_pbar:
            self.display("Starting inference process.", classification=4, Loud=loud)
            inference_pbar.set_description("Pre-processing input data")
            ...
            inference_pbar.set_description("Judge node operations")
            ...
            inference_pbar.set_description("Segment operations")
            bars = {}
            base_pos = 1
                ...
                while active and loop_iteration < max_iter:
                    for node in processing_nodes:
                        ...
                        if scaled_delta is not None:
                            ...
                            if not ret:
                                self.display(f"Processing node at position {node.position} could not forward signal; it may be expired.", 3, Loud=loud)
                                continue
                    active = False
                    active_count = 0
            ...
            for i, segment in enumerate(selected_segments):
                bars[segment['id']] = tqdm(
                    total=3,
                    desc=f"Segment {segment['id']} Inference",
                    position=base_pos + i * 2,
                    leave=True,
            ...
            inference_pbar.set_description("Inference Complete")
            inference_pbar.update(1)
            return final_prediction
            
    def train(self, unprocessed_dataset, epochCount):
        with tqdm(total=3, desc="Training", leave=True) as training_pbar:
            pass
...
if __name__ == "__main__":
    ...
    logger = Logger.Logger(f"app_{int(time.time())}.log", log_level)
    ...
    if mode != "demo":
        ...
        if choice == "1":
            ...
            app.Logger.log("Training mode selected. (Not yet implemented)", 4, True)
        elif choice == "4":
        ...
        app.Logger.log("Will begin training phase now. (Not yet implemented)", 4, True)
        time.sleep(1)
        app.train(dataset, 3)
