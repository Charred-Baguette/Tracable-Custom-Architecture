...
class ProcessingNode:
    ...
    def connect_nearest_nodes(self, node_list, connection_percentage):
        ...
        target_count = max(1, int(math.ceil(connection_percentage * 50)))
    ...
    def forward_signal(self):
        if self.signal is None:
            self.display("No signal to forward. Checking queue", 2)
        ...
        if not viable_nodes:
            self.display("No viable connected nodes to forward the signal.", 3)
            self.signal.signal_life = 0  # Expire the signal (was: .life — wrong attribute)
