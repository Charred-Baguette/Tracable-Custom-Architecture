...
class SegmentHandlerWrapper:
    ...
    def run(self, train_records: list, test_records: list,
        ...
        # ── De-normalise predictions to original scale before computing metrics ──
        # test_records come from ComparisonManager in original scale (e.g. exam_score ~67).
        # SegmentHandler normalises internally, so its predictions emerge in [0,1] space.
        # Only predictions need de-normalisation; actuals are already in original scale.
        tb = getattr(handler, '_target_bounds', None)
        if tb is not None:
            lo, hi = tb
            span = hi - lo
            if span > 1e-9:
                inf_predictions = [p * span + lo for p in inf_predictions]
