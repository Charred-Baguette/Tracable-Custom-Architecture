...
class SegmentHandler:
    ...
    def initializeSegment(self):
        ...
        # Compute sparse connections (used in Phase 3 of the density taper).
        splitter.calculate_nearest_neighbors(self.segmentComponents['processing_nodes'])
        splitter._sparse_connected_nodes = list(splitter.connected_nodes)
    ...
    def segmentInfer(self, pre_processed, loud=True):
        ...
        def _run(bar=None):
            ...
            signal_input = {k: v for k, v in pre_processed.items() if k != self.target}
            for node in processing_nodes:
                if not node.weights:
                    node.initialize_weights(signal_input)
            signals = list(splitter.process(signal_input, self.max_x))
            self.display(
                f"Generated {len(signals)} signals, Splitter has {len(splitter.connected_nodes)} connected nodes",
                classification=4, Loud=loud
            )
            # Seed active set: only nodes that just received a signal from the splitter
            active_nodes = set()
            ...
            # Only nodes holding a signal are scheduled each step.
            # After forwarding, neighbors are checked so the newly activated node
            # enters the next active set — no full scan required.
            _step(f"Segment {self.segment_id} Processing signals")
            loop_iteration = 0
            max_iter       = 500
            num_signals    = len(signals)

            while any(s.is_active() and not s.collected for s in signals) and loop_iteration < max_iter:
                # If tracking lost all active nodes, recover by scanning all processing nodes
                if not active_nodes:
                    active_nodes = {
                        node for node in processing_nodes
                        if node.signal is not None or node.signal_queue
                    }
                next_active = set()
                    ...
                    if scaled_delta is not None:
                        ...
                        if not ret:
                            ...
                            for neighbor in node.connected_nodes:
                                if not hasattr(neighbor, 'review_signals') and (
                                    neighbor.signal is not None or neighbor.signal_queue
                                ):
                ...
                if loop_iteration % 100 == 0 and bar:
                    active_count = sum(1 for s in signals if s.is_active() and not s.collected)
    ...
    REPULSION_STRENGTH         = 0.01   # pairwise node-repulsion force constant
    ...
    def _forward_segment(self, pre_processed):
        ...
        *before* review_signals() clears rev.signals so backprop can use it.
        ...
        loop_iteration = 0
        while any(s.is_active() and not s.collected for s in signals) and loop_iteration < 500:
            # If tracking lost all active nodes, recover by scanning all processing nodes
            if not active_nodes:
                active_nodes = {
                    node for node in processing_nodes
    ...
    def _epoch(self, sample_list, lr_w, lr_p, lr_s, desc,
               epoch=0, epoch_count=1, freeze_connections=False):
        """
        ...
        # ── Signal-density taper schedule ─────────────────────────────
        # Phase 1 (first 20%):  all nodes — full coverage, maximum gradient signal
        # Phase 2 (next ~30%):  half the nodes — reduced but still dense
        # Phase 3 (remaining):  sparse connections from init (current behavior)
        # Constraint: Phase 3 starts no earlier than ceil(50% of epochs).
        n_nodes     = len(processing_nodes)
        phase1_end  = math.ceil(epoch_count * 0.20)
        phase3_start = max(math.ceil(epoch_count * 0.40), math.ceil(epoch_count * 0.50))
        if epoch < phase1_end:
            splitter.connected_nodes = list(processing_nodes)
        elif epoch < phase3_start:
            k = max(1, n_nodes // 2)
            splitter.connected_nodes = random.sample(processing_nodes, k)
        else:
            splitter.connected_nodes = list(
                getattr(splitter, '_sparse_connected_nodes', processing_nodes)
            )
        ...
        def _run_samples(progress=None, task=None):
            ...
            for sample in sample_list:
                ...
                # 2. Position: accumulate gradient only — applied once after the epoch
        ...
        # Pairwise repulsion: inject into position gradients before applying them.
        # Nodes closer than the margin push each other away, preventing collapse
        # and forcing spatial diversity across the quarter-circle.
        repulsion_margin = self.max_x / math.sqrt(max(len(processing_nodes), 1))
        for node_i in processing_nodes:
            for node_j in processing_nodes:
                if node_j is node_i:
                    continue
                d = math.dist(node_i.position, node_j.position)
                if 1e-9 < d < repulsion_margin:
                    for k in range(len(node_i.position)):
                        direction = node_i.position[k] - node_j.position[k]
                        node_i.position_gradient[k] -= (
                            self.REPULSION_STRENGTH * direction / (d ** 3)
                        )

        # Normalise by sample count so LR semantics match per-sample SGD:
        # step ≈ lr * mean_gradient, not lr * sum_of_16000_gradients.
        n_samples = max(len(sample_list), 1)
        for node in processing_nodes:
            node.position_gradient = [g / n_samples for g in node.position_gradient]

        # Apply batch-accumulated position gradient once per epoch, then reconnect.
        for node in processing_nodes:
    ...
    def _run_final_test_eval(self, best_epoch, test_list, epoch_history=None,
        ...
        # De-normalise to original scale for magnitude-sensitive metrics
        # (MAPE, within-X%, MAE, RMSE).  R² and direction accuracy are
        # scale-invariant and use the normalised values unchanged.
        tb = getattr(self, '_target_bounds', None)
        if tb is not None:
            lo, hi = tb
            span = hi - lo
            if span > 1e-9:
                predictions = [p * span + lo for p in predictions]
                actuals     = [a * span + lo for a in actuals]
    ...
    def _eval_avg_error(self, test_list):
        ...
        for sample in test_list:
            ...
            if tb is not None:
                ...
                span = hi - lo
                if span > 1e-9:
                    avg_pred   = avg_pred   * span + lo
                    target_val = target_val * span + lo
            if target_val == 0:
                continue
    ...
    def train(self, dataset, epoch_count=3):
        """
        ...
        self._target_bounds = preprocessor.feature_bounds.get(self.target)
...
if __name__ == "__main__":
    ...
    epoch_history = []
    pre_reports   = []
    actual_norm   = actual   # overwritten once feature_bounds are available
    ...
    if choice == "load":
        ...
        actual_norm  = pre_input.get(TARGET_COL, actual)   # target in normalised [0,1] space
        pre_reports  = handler.segmentInfer(pre_input, loud=True)
        logger.log(f"Pre-training: {len(pre_reports)} reviewer report(s).", 4, True)

        pre_table = Table(title=f"Pre-Training Reviewer Predictions  (raw actual={actual})",
        ...
        pre_table.add_column("Actual (norm)",     style="bold white", justify="right")
        pre_table.add_column("Error %",           style="red",        justify="right")
        for r in pre_reports:
            pred = r['prediction']
            err  = f"{abs(pred - actual_norm) / abs(actual_norm) * 100:.2f}%" if actual_norm else "—"
            pre_table.add_row(str(r['reviewer_position']), f"{pred:.4f}", f"{actual_norm:.4f}", err)
        logger.console.print(pre_table)

        # ── Train ─────────────────────────────────────────────────────
        logger.console.print(Rule("[bold green]Training[/bold green]"))
        epoch_history = handler.train(dataset, epoch_count=25)
    ...
    if epoch_history:
        best_e = min(
            epoch_history,
            key=lambda e: e['test_avg_error']
                          if not _math.isnan(e.get('test_avg_error', float('nan')))
                          else float('inf')
        )
    ...
    actual_norm  = post_input.get(TARGET_COL, actual)   # normalised target (both paths)
    ...
    cmp_table = Table(
        title=f"Reviewer Predictions vs Actual  (raw={actual}, norm={actual_norm:.4f})",
        box=box.ROUNDED, border_style="cyan", show_lines=True
    )
    ...
    cmp_table.add_column("Actual (norm)",   style="bold white",   justify="right")
    ...
    def _err_pct(pred):
        if pred is None or actual_norm is None or actual_norm == 0:
            return float('nan')
        return abs(pred - actual_norm) / abs(actual_norm) * 100.0
    ...
    for pos in all_pos:
        ...
        cmp_table.add_row(
            str(pos),
            _fmt(pre_pred), _fmt(post_pred), _fmt(actual_norm),
    ...
    cmp_table.add_row(
        "[bold]Average[/bold]", "—", "—", _fmt(actual_norm),
        f"[red]{pre_avg_str}[/red]", f"[green]{post_avg_str}[/green]"
    )

    logger.console.print(cmp_table)

    # ── Epoch 0 baseline from pre-training inference ──────────────────
    epoch0_reviewer_errs = {}
    if pre_reports and actual_norm:
        for r in pre_reports:
            pred = r['prediction']
            if actual_norm != 0:
                epoch0_reviewer_errs[r['reviewer_position']] = (
                    abs(pred - actual_norm) / abs(actual_norm) * 100.0
                )
    ...
    if epoch_history:
        ...
        train_epochs  = [e['epoch'] for e in epoch_history]
        all_positions = sorted({pos for e in epoch_history for pos in e['errors']}
                               | set(epoch0_reviewer_errs.keys()))

        # Prepend epoch 0; test error is unavailable before training (shown as gap)
        epochs = [0] + train_epochs

        fig, ax = plt.subplots(figsize=(10, 6))
        for pos in all_positions:
            errs = [epoch0_reviewer_errs.get(pos, float('nan'))] + \
                   [e['errors'].get(pos, float('nan')) for e in epoch_history]
        ...
        epoch0_avg = (sum(epoch0_reviewer_errs.values()) / len(epoch0_reviewer_errs)
                      if epoch0_reviewer_errs else float('nan'))
        avg_errs = [epoch0_avg] + avg_errs
        ax.plot(epochs, avg_errs, marker='s', linestyle='--', linewidth=2,
                color='black', label='Train Avg')

        test_errs = [float('nan')] + [e.get('test_avg_error', float('nan')) for e in epoch_history]
