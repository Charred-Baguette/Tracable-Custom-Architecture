...
class PreProcesingNode:
    def __init__(self, Logger, logger_classification):
        ...
        self.feature_bounds = {}   # populated by process_dataset; used by normalize_numerical
    ...
    def process_dataset(self, dataset):
        ...
        # Seed per-column bounds from the full dataset so the first samples
        # are not normalised against a degenerate single-point range.
        # normalize_numerical will extend these bounds whenever a value falls
        # outside the observed range, so out-of-distribution inference samples
        # are handled correctly without any external configuration.
        numeric_cols = data.select_dtypes(include='number').columns
        for col in numeric_cols:
            lo, hi = float(data[col].min()), float(data[col].max())
            existing = self.feature_bounds.get(col)
            if existing is None:
                self.feature_bounds[col] = (lo, hi)
            else:
                self.feature_bounds[col] = (min(existing[0], lo), max(existing[1], hi))
    ...
    def normalize_numerical(self, data):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, (list, tuple, pd.Series)):
                ...
                fv = float(value)
                # Retrieve or initialise bounds for this feature, then extend
                # them to cover the current value.  This means out-of-range
                # inference samples never break normalisation — they simply
                # widen the known range for all future samples as well.
                lo, hi = self.feature_bounds.get(key, (fv, fv))
                lo = min(lo, fv)
                hi = max(hi, fv)
                self.feature_bounds[key] = (lo, hi)
                denom = hi - lo
                data[key] = (fv - lo) / denom if denom > 1e-9 else 0.0
